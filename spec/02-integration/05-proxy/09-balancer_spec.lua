...
local function poll_wait_health(upstream_name, localhost, port, idx, value)
  local hard_timeout = 300
  local expire = ngx.now() + hard_timeout
  while ngx.now() < expire do
    local health = get_upstream_health(upstream_name)
    assert.same(localhost .. ":" .. port, health.data[idx].target)
    if health.data[idx].health == value then
      break
    end
    ngx.sleep(0.01) -- poll-wait
  end
    ...
    describe("#healthchecks", function()
      ...
      it("#wait perform active health checks -- up then down", function()
        ...
        for nfails = 1, 3 do
          ...
          poll_wait_health(upstream_name, localhost, port2, 1, "UNHEALTHY")
      ...
      it("#wait perform active health checks -- automatic recovery", function()
        ...
        for nchecks = 1, 3 do
          ...
          -- Give time for healthchecker to detect
          poll_wait_health(upstream_name, localhost, port2, 1, "HEALTHY")
